      ...
      <div className="wo-list-container">
        {isPending && <div className='loading'>Loading...</div>}
        {
          workorders.filter((item) => {
            return search.toLowerCase() === '' ? item.status === "PENDING PAYMENT" :
             (item.status === "PENDING PAYMENT" && item.woNumber.toLowerCase().includes(search.toLowerCase()) || 
              item.status === "PENDING PAYMENT" && item.productName.toLowerCase().includes(search.toLowerCase())
             );
          }).map((wo) => {
            return ( 
                <div className="wo-previwe" id = "delivery-preview" key={wo.id} >
                  ...
                  <div style={{paddingLeft:"40px"}}> {wo.cost} </div>
                  <div id = "delivery-complete-button">
                    <button onClick={updateWorkorder(wo)}>COMPLETE PAYMENT</button>  
                  </div>
                </div>
            );
          })
...
export default withRouter(PaymentUpdate);